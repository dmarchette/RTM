...
Latent semantic indexing (LSI) is a way of reducing the 
dimensionality\index{dimensionality reduction}
...
qfcorp <- Corpus(VectorSource(docs))

dtmQF.TfIdf <- DocumentTermMatrix(qfcorp, control = dtm.control)
dtmQF.TfIdf <- removeSparseTerms(dtmQF.TfIdf, 0.99)
dim(dtmQF.TfIdf)
@

The basic tool of LSI is the 
...
<<lsiworddists,cache=TRUE,tidy=FALSE,dependson=c('lsi1','lsiTM')>>=
...
dtmQF.Tf <- DocumentTermMatrix(qfcorp, control = dtm.control)
dim(dtmQF.Tf)
...
method of \cite{zhu:2006}, which uses a 
profile likelihood\index{dimensionality reduction!profile likelihood}
to pick the embedding dimension.
...
Scorp <- Corpus(VectorSource(Sdocuments))

dtmS.Tf <- DocumentTermMatrix(Scorp, 
   control = list(
     tolower = TRUE,
     removePunctuation = TRUE,
     removeNumbers = TRUE,
...
dtmS.TfIdf <- DocumentTermMatrix(Scorp, 
   control = list(
     tolower = TRUE,
     removePunctuation = TRUE,
     removeNumbers = TRUE,
...
dissimilarity measures (the library 
\texttt{proxy}\index{R Libraries!proxy@\texttt{proxy}}
implements a large
number).
...
\section{Probablistic Latent Semantic Indexing}
...
\index{R Libraries!topicmodels@\texttt{topicmodels}}
...
\begin{figure}
\centering
<<qfinTM1fig,echo=FALSE,cache=TRUE,results='hide',dependson=c('topicsTM','qfinTM')>>=
...
\index{R Libraries!wordcloud@\texttt{wordcloud}}
...
the 
Akaike Information Criterion\index{Akaike Information Criterion}
(AIC)\index{AIC|see {Akaike Information Criterion}}
-- to determine the proper number
...
<<qfinTM2,cache=FALSE,echo=c(1:5,10),tidy=FALSE,dependson='topicsTM'>>=
...
if(file.exists("SupportData/qfinTM2_models.RData")){
ks <- 2
burnin <- 1
iter <- 1
}
...
if(file.exists("SupportData/qfinTM2_models.RData")){
  load("SupportData/qfinTM2_models.RData")
} else {
   save(models,ks,burnin,iter,file="SupportData/qfinTM2_models.RData")
}
@

Then we compute the AIC for these (see Figure \ref{fig:qftmAIC}). 
The point at which the AIC
is maximal is the ``best''
choice for topics.
\index{R Libraries!Rmpfr@\texttt{Rmpfr}}

<<qfinTM3,cache=TRUE,tidy=FALSE>>=
library(Rmpfr)
...
\begin{figure}
\centering
<<qfinAICfig,echo=FALSE,results='hide',dependson='qfinTM3'>>=
...
<<qfinModel1,cache=TRUE,tidy=FALSE,dependson='topicsTM'>>=
set.seed(12345)
...
<<qfinModel2,cache=TRUE,tidy=FALSE,dependson='topicsTM'>>=
set.seed(21545)
...
\section{Correlated Topic Models}
